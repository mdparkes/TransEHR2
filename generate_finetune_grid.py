"""Expand a grid or a set of seed repeats as standalone experiment configs.

Two shapes:

    grid          --rates and --half_lives, crossed, over one shared encoder named by
                  --encoder. Every cell links the same weights, so the only thing separating
                  them is the finetuning settings.
    seed repeats  --seeds, one run per seed at a single configuration. Give two groups the
                  same seeds and their comparison becomes within-seed. Without --encoder each
                  repeat pretrains its own encoder under its own seed, which is what a repeat
                  has to do to resample anything the pretraining settings act on.
...
def seed_cells(prefix, rate, half_life, seeds, seed_pretraining):
    ...
    Args:
        ...
        seed_pretraining: Whether the seed also varies pretraining. It has to, for a repeat
            to resample anything a pretraining hyperparameter acts on; a linked encoder is
            one draw, and repeats over it only resample the head and the batch order.

    Yields:
        Tuples of (experiment name, config overrides).
    """
    for seed in seeds:
        overrides = {'FINETUNE_LEARNING_RATE': rate,
                     'FINETUNE_LR_HALF_LIFE': half_life,
                     'FINETUNE_SEED': seed}
        if seed_pretraining:
            overrides['PRETRAIN_SEED'] = seed
        yield f'{prefix}_seed{seed}', overrides
...
def main():
    ...
    parser.add_argument('--encoder', default=None,
                        help='EXPERIMENT_NAME of a pretrain whose weights every cell links. '
                             'Omit and each cell pretrains its own, which --seeds then '
                             'varies -- the only way a repeat resamples pretraining.')
    ...
    if args.encoder is None and seeds is None:
        parser.error('Without --encoder every cell pretrains its own encoder, which is only '
                     'worth the GPU time when --seeds varies them.')
    ...
    plan = (seed_cells(args.prefix, rates[0], half_lives[0], seeds, args.encoder is None)
    ...
    step = 1
    if args.encoder:
        print(f'\n{step}. Link the encoder into every cell, so each skips pretraining and they')
        print('   all finetune from identical weights:')
        ...
        print(f'\n{step}. Pretrain and finetune every cell. Nothing is linked, so each one '
              f'trains')
        print('   its own encoder under its own seed:')
    print(f'   for cfg in {args.output_dir}/{args.prefix}_{suffix}.yaml; do')
